...
export function HistorialCambios({ tabla, filaId, profiles, limit = 200, compacto }: Props) {
  ...
  if (error) return <p className="text-secondary text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">{error}</p>

  return (
    <div>
      {items.length === 0 && !cargando && (
        <p className="text-secondary text-slate-500 italic py-3 flex items-center gap-1.5"><History className="w-3.5 h-3.5" /> Sin cambios registrados.</p>
      )}
      <div className="divide-y divide-slate-100">
        {items.map(e => {
          const cambios = e.accion === 'UPDATE'
            ...
            : []
          return (
            <div key={e.id} className="py-2 text-secondary">
              <div className="flex items-center gap-1.5 flex-wrap text-slate-600">
                ...
                {!compacto && (
                  <span className="text-slate-500">
                    <span className="font-mono text-badge bg-slate-100 rounded px-1">{e.tabla}</span>
                    {' '}<span className="font-medium text-slate-700" title={e.filaId}>{nombreFila(e)}</span>
                  </span>
                )}
                <span className="text-slate-500 ml-auto text-meta" title={new Date(e.at).toLocaleString('es-ES')}>{fechaRelativa(e.at)}</span>
              </div>
              {cambios.length > 0 && (
                <ul className="mt-1 ml-5 space-y-0.5">
                  {cambios.map(([campo, [a, b]]) => (
                    <li key={campo} className="text-meta text-slate-500 break-words">
      ...
      {cargando && <p className="text-secondary text-slate-500 py-2">Cargando…</p>}
      {!fin && !cargando && items.length > 0 && (
        <Button size="sm" className="mt-2" onClick={() => void cargar(items[items.length - 1].at)}>
          Cargar más
        </Button>
      )}
    </div>
