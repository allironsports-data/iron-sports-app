...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    items: [
      ...
      'Ficha del partido: la app avisa cuando el equipo que un jugador tiene en su ficha no es ninguno de los dos que juegan, y ofrece corregirlo de un clic. Si es del mismo club pero otra categoría (un Juv B en un partido del filial) propone directamente el equipo correcto; si es de un club que no juega ese partido, o no tiene equipo puesto, ofrece los dos. Sale al vincular un jugador nuevo y también, con el botón «Revisar», para repasar de golpe los que ya estaban vinculados con el equipo desactualizado',
      'Pipeline sale de Captación y pasa a ser una sección propia, «Pipeline», en la barra de arriba (y en la barra inferior del móvil). Dentro está el tablero Firmar de siempre, sin tocar nada, y una vista nueva',
      'Pipeline → nueva pestaña «Por zona»: la vista por zonas ya existía, pero estaba escondida en un selector pequeño dentro del tablero. Ahora es una pestaña propia, y el selector interno se queda con Estatus y Encargado. Además cambia de forma: las zonas pasan a una lista lateral (una tira arriba en móvil) con su recuento, el reparto por estatus y en rojo cuántas llevan demasiado sin tocarse — antes era una rejilla de tarjetas que empujaba el tablero fuera de la pantalla',
      'Pipeline → nueva pestaña «Avisos»: los avisos cruzados con el resto de la app (le vieron en un partido, cambió de club, informe nuevo, contrato acabando, sin encargado…) dejan de estar en un desplegable encima del tablero, donde había que abrirlo y luego abrir cada grupo. Ahora todos los grupos se ven a la vez en dos columnas, con lo urgente delante, un botón para quedarte solo con lo que requiere acción, y el número de avisos en la propia pestaña',
      'Pipeline → nueva pestaña «Por encargado»: el panel de trabajo de cada uno. Los firmados no salen (ni en los bloques, ni en el embudo, ni en la tabla: ya no son trabajo, solo se cuentan arriba), y cada tarjeta aparece en un único bloque —el más urgente que le toque— para que los números signifiquen algo. Cuatro bloques con lo que urge — qué toca hoy (acciones vencidas o de hoy), se están enfriando (las que pasan la cadencia de su estatus), mis calientes y las que esperan teléfono — y debajo el pipeline entero: un embudo por estatus que al pulsarlo filtra, y una tabla con estatus, zona, días parada y próxima acción, ordenable y descargable a CSV. Se abre en el tuyo y hay un desplegable para ver el de cualquier compañero',
      'Navegación: el botón «atrás» del navegador hace lo que uno espera. Abrir una ficha de scouting, un partido, una tarjeta de Firmar o una tarea y pulsar atrás cierra eso y te deja donde estabas — antes te sacaba de la sección entera. Las pestañas de segundo nivel (Captación → Partidos, Pipeline → Avisos, Distribución → Clubes, Mantenimiento → Equipo…) van ahora en la dirección, así que atrás también se mueve entre pestañas, y al volver a una sección la encuentras en la pestaña que dejaste',
      'Navegación: los enlaces llegan a donde dicen. Las «acciones de Firmar» del Dashboard, de Mi día y del buscador ⌘K abren la tarjeta en Pipeline (antes mandaban a Captación y la tarjeta no se abría); un resultado de tarea en ⌘K abre la tarea; «ir a Firmar» desde una ficha de scouting ya no deja Captación con el cuerpo vacío al volver; los enlaces de cabecera cierran la ficha o el club que tuvieras abierto (pulsar «Captación» con un club abierto seguía enseñando Distribución); Admin se abre desde cualquier sitio, también desde la ficha de un jugador, y al cerrarlo vuelves a donde estabas en vez de a Mantenimiento; y Distribución tiene por fin su enlace a Pipeline',
      'Navegación: los enlaces compartidos vuelven a funcionar. Pegar #/jugador/… o #/pipeline/avisos en una pestaña nueva abría siempre Mantenimiento porque la app pisaba la dirección con lo último guardado antes de leerla. Y #/pipeline no estaba registrado: ni compartirlo ni volver atrás hacia él hacía nada',
      'Mantenimiento → Jugadores: la vista de lista pasa a ser la de entrada, en vez de las tarjetas — con la cartera entera caben muchos más jugadores sin bajar. Las tarjetas y la tabla siguen ahí, y la app recuerda la que uses',
      'Mantenimiento → Jugadores: cada jugador tiene ahora un estado — Activo (lo gestionamos nosotros), Inactivo (nos ha dejado o el contrato de representación ha vencido) y Gestión partner (el día a día lo lleva el partner). Se pone desde «Editar ficha», y arriba del todo hay un filtro con el número de cada uno. **La lista arranca enseñando solo los Activos**; los otros se ven pulsando su botón o «Todos», donde los no activos llevan su chapa de color. Solo afecta a esta lista: un inactivo sigue saliendo en tareas, cumpleaños y buscador. Requiere ejecutar migration_player_estado.sql; mientras tanto todos se leen como activos y las fichas se siguen guardando igual',
      'Mantenimiento → Jugadores: nuevo filtro de Partner, en botones grandes arriba del todo (Todos · Toldra · Boulema · Sin partner) con el número de jugadores de cada uno. La lista de partners sale de los propios datos, así que si mañana entra un tercero aparece solo; y «Toldra», «Toldrá» y «toldra» cuentan como el mismo, enseñando la grafía más usada. Se combina con el buscador y con el resto de filtros, el desplegable de encargados pasa a contar solo dentro del partner elegido, y la elección se recuerda al cambiar de pantalla',
      'Tareas: nuevo selector «Todas · Mantenimiento · Captación» encima del tablero, con el número de cada uno. Captación son las próximas acciones que nacen de una tarjeta del pipeline de Firmar (llamar, conseguir teléfono, reunión…); Mantenimiento, todo lo demás. Se reconocen por el vínculo real con la tarjeta, así que siguen bien clasificadas aunque cambies el título o el tipo de la tarea. El filtro afecta también a los contadores de arriba (vencidas, hoy, esta semana, en progreso) y se recuerda al cambiar de pantalla',
      'Pipeline → Firmar: al abrir una tarjeta, debajo del jugador de Captación vinculado salen ya sus tres últimos informes — fecha, scout, veredicto, título y las dos primeras líneas. Al pulsar uno se despliega el texto entero ahí mismo, y si tiene más hay un «Ver los N» que abre la ficha. Antes había que salir a Captación, abrir la ficha y volver solo para leer un informe',
      'Pipeline → Avisos: cada aviso se puede resolver sin salir de la lista. «Posponer 7 días» (reloj) lo esconde y lo devuelve solo cuando pasa la semana; «Visto» (check) lo esconde mientras diga exactamente lo mismo, y lo devuelve si cambia — si el contrato pasa a otra fecha o el jugador cambia de club, vuelve a avisar. El contador de la pestaña baja al momento, y abajo hay un enlace para volver a ver los que has escondido. Es tuyo y de este navegador: no afecta a nadie más',
      'Pipeline → Firmar: cada tarjeta lleva ahora los días que lleva sin tocarse («12d») en vez de solo un punto de color — en rojo si ha pasado la cadencia de su estatus (caliente 10 días, templado 50, frío 90), en ámbar si está cerca, en gris si va bien. Antes el punto avisaba de que algo iba mal pero había que abrir la ficha para saber cuánto',
      'Pipeline → nueva vista «Timeline»: todo lo que se mueve en el pipeline de más reciente a más antiguo — apuntes (notas, llamadas, whatsapps, reuniones, entorno), cambios de estatus, jugadores que entran, firmas y las tareas que nacen de una próxima acción. Agrupado por día, con filtros por tipo, por persona y buscador. Al pulsar un movimiento se abre su tarjeta en Firmar',
      'Admin → nueva pestaña «Control de equipos»: cuadro de cobertura. Arriba, equipos, relevantes, % cubiertos, jugadores en Llamar y en Seguir. Debajo, el desglose por zona o por categoría (cobertura, jugadores, en Llamar y el reparto de etiquetas) ordenado por dónde hay más gente en Llamar, descargable a CSV. Y dos listas: los equipos con más jugadores en Llamar, y los relevantes que se están quedando sin cubrir. Con su propio selector de temporada o histórico',
      'Ficha del equipo: cuando la ★ o la ✓ vienen de una regla y no las has puesto tú, ahora se explica por qué («sale como relevante: 2 jugadores en Llamar») y los jugadores que cuentan llevan un punto ámbar. Antes podías ver un equipo marcado como relevante con todos sus jugadores sin etiqueta y no había forma de entenderlo: contaban por tener informes con veredicto Llamar',
      'Captación → Equipos: dos reglas automáticas. Un equipo con 2 o más jugadores «en Llamar» se marca solo como ★ relevante, y uno visto en 2 o más partidos como ✓ cubierto. Las marcas deducidas salen en un tono más claro para distinguirlas de las que pones tú, y arriba aparece «N equipos cumplen alguna regla — Aplicar» para guardarlas de golpe cuando quieras (hasta entonces solo se ven, no se toca nada)',
      'Un jugador cuenta como «en Llamar» si tiene esa etiqueta o si tiene algún informe con veredicto Llamar, y cuenta una sola vez aunque cumpla las dos cosas',
      'El recuento de arriba («X de Y relevantes cubiertos») y la columna ✓ de la tabla ya usan el mismo criterio: antes el resumen daba por cubierto cualquier equipo con un solo partido y la columna solo miraba la marca manual, así que los dos números no cuadraban',
      'Otra tanda de retoques de interfaz: los emojis que quedaban como icono en Conclusiones (candidatos, mapa, movimientos, aviso de «Decidir» parado, filtro de zona), Equipos (Zonas, Actualizar plantilla) y la lista de clubes (notas, negociaciones estancadas) pasan a iconos; las flechas ← → de cambiar semana en Tareas y Partidos también, y ahora dicen «semana anterior / siguiente» para quien navega con lector de pantalla',
  ...
  },
  {
    ...
    items: [
      ...
      'Planificación: clic en el partido para corregir local/visitante sin abrir la ficha completa (con enlace directo a la ficha si hace falta tocar algo más). En «Persona» ya se puede quitar cualquier nombre con la ✕, sea o no un perfil del sistema, y añadir cualquier nombre suelto (no solo los perfiles)',
      'Ficha del jugador (Mantenimiento y Captación): botón «Exportar informe» (icono ⬇, junto a Editar) que genera un PDF de una página con la ficha deportiva y todos los informes — listo para compartir. En Mantenimiento incluye contrato, informes de partido y notas de rendimiento; en Captación, la ficha y las observaciones. Ninguno de los dos lleva assessment, quién escribió cada informe ni datos personales (teléfono, familia…)',
      'Informe de Captación: ahora incluye arriba un resumen escrito por IA que sintetiza todas las observaciones sueltas de ese jugador en un párrafo (requiere desplegar la Edge Function «resumen-scouting» y configurar la clave de Anthropic — ver el propio archivo). Si falla o no está configurada, el informe se genera igual, solo que sin esa sección',
      'Distribución: al cerrar una negociación con un club, se anota solo un evento «Transferencia» en la Actividad del jugador (Mantenimiento) y sale un aviso para repasar a mano su club y contrato — no los cambia automáticamente',
      'Distribución: nueva temporada 2026-27, seleccionable en un desplegable junto al título (2025-26 queda como «archivada»: se sigue consultando tal cual, pero ya no recibe altas). Los clubes de 2025-26 se han copiado a 2026-27 (mismo nombre, liga, país, contacto y encargado) sin arrastrar solicitudes ni el marcador de contactado; los jugadores empiezan de cero en la temporada nueva. La pestaña Distribución de la ficha del jugador también tiene ahora su propio selector de temporada, para no mezclar situaciones de 25/26 y 26/27 en la misma lista',
      'Menos espera al entrar: cada tabla de fondo (clubes, informes, partidos…) ya aparece en cuanto llega, en vez de esperar a que las 16 terminen juntas; el aviso «Sincronizando datos…» solo sale si de verdad tarda más de medio segundo',
