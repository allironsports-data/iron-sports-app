...
import { CalendarCheck, CheckCircle2, ExternalLink, Eye, Phone, ClipboardList, Trophy, ListTodo, AlertTriangle, Sun } from 'lucide-react'
...
const ICONO: Record<MiDiaTipo, { Icon: typeof Trophy; cls: string; label: string }> = {
  ...
  accion: { Icon: Phone, cls: 'bg-amber-50 text-amber-600', label: `Acción ${L.firmar}` },
...
export function MiDia({
  ...
  onCompleteTask, onSetMatchSeen, onCompleteFirmasAction,
}: MiDiaProps) {
  const hoy = hoyISO()
  ...
  function accionPrincipal(it: MiDiaItem) {
    ...
    if (it.tipo === 'partido' && it.ref.matchId) {
      ...
      return (
        <Button variant="primary" icon={<Eye />} loading={disabled} onClick={() => run(it.id, () => onSetMatchSeen(id))}
    ...
    if (it.tipo === 'accion' && it.ref.firmasEntryId) {
      ...
      return (
        <Button variant="primary" icon={<CheckCircle2 />} loading={disabled} onClick={() => run(it.id, () => onCompleteFirmasAction(id))}
    ...
    if (it.tipo === 'tarea' && it.ref.taskId) {
      ...
      return (
        <Button variant="primary" icon={<CheckCircle2 />} loading={disabled} onClick={() => run(it.id, () => onCompleteTask(id))}
          className="bg-blue-600 hover:bg-blue-700 disabled:hover:bg-blue-600">
          Hecho
        </Button>
      )
    }
  ...
  return (
    <div className="mx-auto max-w-2xl">
      {/* Título de página (la navegación la pone AppShell) */}
      <div className="flex items-center gap-3 mb-4">
        <div className="flex-1 min-w-0">
          <h1 className="text-base font-bold text-slate-800 truncate">
            {L.miDia} · <span className="font-medium text-slate-600">{tituloDia(hoy)}</span>
          </h1>
          <p className="text-secondary text-slate-500 truncate">
            {resumen}
            {c.vencidos > 0 && <span className="ml-1 text-red-600 font-medium">· {c.vencidos} {c.vencidos === 1 ? 'vencido' : 'vencidos'}</span>}
          </p>
        </div>
        {isAdmin && profiles.length > 1 && (
          <Select value={personaId} onChange={e => setPersonaId(e.target.value)} aria-label="Persona" className="w-auto max-w-[10rem]">
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          subtitle={`Ni partidos, ni acciones de ${L.firmar}, ni tareas vencidas. Buen momento para adelantar trabajo.`}
        />
      ) : (
            ...
            return (
              <li key={it.id}
                ...
                <div className="flex items-start gap-3">
                  <div className={`shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${cls}`} title={label} aria-label={label} role="img">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-2">
                      ...
                      {it.vencido && (
                        <Badge tone="danger"><AlertTriangle className="w-3 h-3" aria-hidden="true" /> Vencido</Badge>
                      )}
                    </div>
                    <p className="text-body font-semibold text-slate-800 leading-snug mt-0.5 break-words">{it.titulo}</p>
                    <p className="text-secondary text-slate-500 mt-0.5 break-words">{it.subtitulo}</p>
                  </div>
                </div>
                <div className="flex gap-2 mt-3">
                  {accionPrincipal(it)}
                  <Button icon={<ExternalLink />} onClick={() => abrir(it)} className="flex-1 bg-slate-100 border-slate-100 text-slate-700 hover:bg-slate-200">
                    Abrir
                  </Button>
                </div>
              </li>
      ...
      {items.length > 0 && (
        <p className="text-meta text-slate-500 text-center mt-6 flex items-center justify-center gap-1">
          <CalendarCheck className="w-3 h-3" aria-hidden="true" /> Lo completado desaparece de la lista
        </p>
      )}
    </div>
  )
