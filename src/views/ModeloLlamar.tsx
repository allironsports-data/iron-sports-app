  ...
  if (!result.ok) {
    return (
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-slate-800 flex items-center gap-2"><Brain className="w-4 h-4 text-slate-500" /> Modelo de «Llamar»</h3>
        <p className="mt-2 text-xs text-slate-500">{result.reason}</p>
        <button onClick={reentrenar} className="mt-3 inline-flex items-center gap-1 text-badge text-slate-500 hover:text-slate-700 border border-slate-200 rounded-md px-2 py-1">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <h3 className="text-sm font-semibold text-slate-800 flex items-center gap-2">
            <Brain className="w-4 h-4 text-slate-500" /> Modelo de «Llamar»
          </h3>
          <button onClick={reentrenar} title="Vuelve a entrenar aunque no hayan cambiado los informes"
            className="inline-flex items-center gap-1 text-badge text-slate-500 hover:text-slate-700 border border-slate-200 rounded-md px-2 py-1">
            <RefreshCw className="w-3 h-3" /> Reentrenar
          </button>
        </div>
        ...
        <p className="mt-1.5 text-badge text-slate-500 leading-relaxed">
        ...
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className="bg-slate-50 rounded-lg px-3 py-2">
              ...
              <div className="text-badge text-slate-500 leading-tight">{s.label}</div>
            </div>
          ))}
        </div>
        <div className="mt-2 flex items-center gap-2 flex-wrap">
          <span className={`text-badge font-semibold px-2 py-0.5 rounded-full border ${calidad.cls}`}>{calidad.label}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {result.top.map(t => (
              <span key={t.term} className="inline-flex items-center gap-1 text-badge bg-red-50 text-red-700 border border-red-100 rounded-full px-2 py-0.5"
                title={`Peso ${t.w.toFixed(2)} · aparece en ${t.df} informes`}>
                {t.term}<span className="text-badge text-red-400">{t.df}</span>
              </span>
            ))}
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {result.bottom.map(t => (
              <span key={t.term} className="inline-flex items-center gap-1 text-badge bg-slate-50 text-slate-600 border border-slate-200 rounded-full px-2 py-0.5"
                title={`Peso ${t.w.toFixed(2)} · aparece en ${t.df} informes`}>
                {t.term}<span className="text-badge text-slate-500">{t.df}</span>
              </span>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        ...
        <p className="text-badge text-slate-500 mt-0.5">Informes marcados Seguir cuyo texto se parece al de un Llamar. Para revisarlos, no para cambiarlos solos.</p>
        <div className="mt-2 divide-y divide-slate-50">
          {seguirCalientes.length === 0 && <p className="text-xs text-slate-500 italic py-2">No hay «Seguir» con texto suficiente.</p>}
          {seguirCalientes.map(({ r, p }) => (
            <div key={r.id} className="flex items-center gap-2 py-1.5">
              <span className={`text-badge font-bold tabular-nums w-10 text-right ${p >= 0.6 ? 'text-red-600' : p >= 0.4 ? 'text-amber-600' : 'text-slate-500'}`}>{pct(p)}</span>
              <span className="min-w-0 flex-1">
                <span className="block text-meta font-semibold text-slate-800 truncate">{nombre(r)}</span>
                <span className="block text-badge text-slate-500 truncate">{scout(r)} · {fecha(r)} · {(r.texto ?? '').slice(0, 90)}…</span>
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <h4 className="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
          <AlertTriangle className="w-3.5 h-3.5 text-slate-500" /> «Llamar» que el texto no sostiene
        </h4>
        <p className="text-badge text-slate-500 mt-0.5">Puede ser un informe escrito corto y de carrerilla, o un Llamar por motivos que no están escritos (contexto, encargo, precio).</p>
        <div className="mt-2 divide-y divide-slate-50">
          {llamarFlojos.map(({ r, p }) => (
            <div key={r.id} className="flex items-center gap-2 py-1.5">
              <span className="text-badge font-bold tabular-nums w-10 text-right text-slate-500">{pct(p)}</span>
              <span className="min-w-0 flex-1">
                <span className="block text-meta font-semibold text-slate-800 truncate">{nombre(r)}</span>
                <span className="block text-badge text-slate-500 truncate">{scout(r)} · {fecha(r)} · {(r.texto ?? '').slice(0, 90)}…</span>
              </span>
      ...
      {/* Probador */}
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        <h4 className="text-xs font-semibold text-slate-700 flex items-center gap-1.5"><FlaskConical className="w-3.5 h-3.5 text-slate-500" /> Probar un informe</h4>
        <p className="text-badge text-slate-500 mt-0.5">Pega aquí el texto de un informe nuevo y te dice qué probabilidad de «Llamar» le ve.</p>
        <textarea
          value={probe}
        ...
        {probeP && (
          <div className="mt-2 flex items-start gap-3 flex-wrap">
            ...
            <div className="text-badge text-slate-500 flex-1 min-w-[200px]">
              ...
              <span className="flex flex-wrap gap-1">
                {probeP.hits.map(h => (
                  <span key={h.term} className={`px-1.5 py-0.5 rounded-full border text-badge ${h.w > 0 ? 'bg-red-50 text-red-700 border-red-100' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        ...
        <p className="text-badge text-slate-500 mt-0.5">De los informes a los que dijo «x%», cuántos acabaron siendo Llamar de verdad. Cuanto más parecidas las dos columnas, mejor.</p>
        <table className="mt-2 w-full text-badge">
          <thead>
          ...
          <tbody>
            {result.calib.map(c => (
              <tr key={c.bucket} className="border-t border-slate-50">
                ...
                <td className={`font-semibold ${c.n ? 'text-slate-700' : 'text-slate-500'}`}>{c.n ? pct(c.real) : '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
