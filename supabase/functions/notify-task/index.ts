...
// Escapa contenido de usuario antes de interpolarlo en el HTML del email
// (evita inyección de HTML/links maliciosos vía títulos, descripciones o comentarios)
...
serve(async (req) => {
  ...
  if (!type || !table || !record) return new Response("ok");
  ...
  if (table === "tasks" && type === "INSERT") {
    ...
    const [user, profile] = await Promise.all([
      getProfile(task.assignee_id as string),
      getProfileRow(task.assignee_id as string),
    ]);
    ...
    const subject = `Nueva tarea asignada: ${String(task.title ?? "")}`;
    const html = `
      <div style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:20px">
        <h2 style="color:#1e3a5f">Hola ${esc(profile?.name)},</h2>
        <p>Se te ha asignado una nueva tarea en <strong>All Iron Sports</strong>:</p>
        <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px;margin:16px 0">
          <p style="margin:0 0 8px"><strong>Tarea:</strong> ${esc(task.title)}</p>
          ${task.description ? `<p style="margin:0 0 8px;color:#64748b">${esc(task.description)}</p>` : ""}
          ${playerName ? `<p style="margin:0 0 8px"><strong>Jugador:</strong> ${esc(playerName)}</p>` : ""}
          <p style="margin:0"><strong>Prioridad:</strong> ${task.priority === "alta" ? "⚡ Alta" : task.priority === "media" ? "Media" : "Baja"}</p>
          ${task.due_date ? `<p style="margin:8px 0 0"><strong>Fecha límite:</strong> ${new Date(task.due_date as string).toLocaleDateString("es-ES")}</p>` : ""}
        </div>
  ...
  if (table === "task_comments" && type === "INSERT") {
    ...
    // Get author name
    const authorProfile = await getProfileRow(comment.author_id as string);
    ...
    const subject = `Nuevo comentario en "${String(task.title ?? "")}"`;
    const html = `
      <div style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:20px">
        <h2 style="color:#1e3a5f">Nuevo comentario</h2>
        <p><strong>${esc(authorProfile?.name ?? "Alguien")}</strong> ha comentado en la tarea <strong>${esc(task.title)}</strong>${playerName ? ` (${esc(playerName)})` : ""}:</p>
        <div style="background:#f0f4ff;border-left:3px solid #3b82f6;padding:12px 16px;margin:16px 0;border-radius:0 8px 8px 0">
          <p style="margin:0;color:#1e293b">${esc(comment.content)}</p>
        </div>
        <p style="color:#94a3b8;font-size:12px">All Iron Sports · Gestión de jugadores</p>
      </div>`;
